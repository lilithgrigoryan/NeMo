...
from nemo.collections.speechlm2.models.salm_asr_timestamping import SALMWithTimestamping
...
class SalmEvalConfig:
    ...
    test_manifest: str
    batch_size: int = 64
    viterbi_batch_size: int = 4
    max_new_tokens: int = 128
    output_manifest: str = "generations_with_timestamps.jsonl"
    verbose: bool = True
    device: str = "cuda"
    dtype: str = "bfloat16"
    extra_eos_tokens: Optional[list[str]] = None
    system_prompt: Optional[str] = None
    user_prompt: Optional[str] = "Transcribe the following:"
    timestamp_type: str = "all"
...
def main(cfg: SalmEvalConfig):
    ...
    model = (
        SALMWithTimestamping.from_pretrained(
            cfg.pretrained_name,
            ctc_model_path=cfg.ctc_model_path,
            share_encoder=cfg.share_encoder,
            share_preprocessor=cfg.share_preprocessor,
        )
        .eval()
    ...
    if cfg.share_encoder:
        sharing_info = model.verify_encoder_sharing()
        logging.info(sharing_info['message'])

    conversations = (
        guess_parse_cutset(cfg.test_manifest)
        .map(
            partial(
                cut_to_conversation,
    ...
    for batch_idx, batch in enumerate(dloader):
        ...
        ts = perf_counter()
        answer_ids, hypotheses = model.generate_with_timestamps(
            ...
            batch_size=cfg.viterbi_batch_size,
            timestamp_type=cfg.timestamp_type,
        )
        ...
        for conv, ctx, ans, hyp in zip(batch["conversations"], batch_contexts, batch_answers, hypotheses):
            ...
            conv_dict['pred_text'] = hyp.text
            conv_dict['timestamp'] = hyp.timestamp
